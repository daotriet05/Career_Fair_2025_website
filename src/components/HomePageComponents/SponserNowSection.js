...
const GnSCard = ({ image, title }) => {
  ...
  return (
    <div className="w-[300px] flex flex-col items-center justify-self-center">
      ...
      {/*<p className="text-center font-bold text-[#737373] mt-2">{title}</p>*/}
    </div>
  );
};
...
const ExhiCard = ({ image, title }) => {
  const [isLoaded, setIsLoaded] = useState(false);
  return (
    <div className="w-[100px] sm:w-[200px] flex flex-col items-center justify-self-center">
      <div className="bg-white w-full h-[140px] rounded-xl overflow-hidden relative">
        {!isLoaded && <div className="absolute inset-0 bg-gray-300 animate-pulse" />}
        <img
          src={image}
    ...
    >
      <div className="w-full max-w-7xl mx-auto py-10">
        ...
        <div className="py-16 grid grid-cols-1 sm:grid-cols-2 gap-x-[390px] gap-y-[70px]">
          {logoOrder.slice(0, 2).map((logo, idx) => (
            <a
              key={idx}
              href={logo.url}
              target="_blank"
            ...
            >
              <GnSCard image={logo.image} title={logo.alt} />
            </a>
          ))}
        </div>
        ...
        <div className="py-16 grid grid-cols-1 sm:grid-cols-2 gap-x-[390px] gap-y-[70px]">
          {logoOrder.slice(2, 4).map((logo, idx) => (
            <a
              key={idx}
              href={logo.url}
              target="_blank"
        ...
        <div className="py-8 grid grid-cols-3 sm:grid-cols-4 gap-x-[100px] gap-y-[10px] sm:gap-y-[50px]">
          {logoOrder.slice(4, 26).map((logo, idx) => (
            <a
              key={idx}
              href={logo.url}
              target="_blank"
              ...
              className="transition hover:scale-110"
            >
              <ExhiCard image={logo.image} title={logo.alt} />
            </a>
          ))}
        </div>
      </div>
