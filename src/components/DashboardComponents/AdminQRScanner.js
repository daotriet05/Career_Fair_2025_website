...
const AdminQRScanner = () => {
  ...
  const stopCamera = () => {
    ...
    try {
        ...
        const student = studentSnap.data(); // 👈 original state (before update)
        ...
        boothCount = Object.values(boothData).filter(Boolean).length;
        const thresholds = [5, 8, 13, 18];
        const received = { ...(student.receivedRewards || {}) }; // clone old

        thresholds.forEach((t) => {
            if (giveMaxRewards && boothCount >= t && !received[t]) {
            received[t] = true; // ✅ mark newly granted
            }
        });
        ...
        // 👉 Pass original + updated separately so UI can distinguish
        await updateDoc(studentRef, update);
        setStudentData({
        ...student,                  // original
        updatedRewards: newReceived // new state (for optional use)
        });
    } catch (err) {
