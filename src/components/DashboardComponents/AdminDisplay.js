// AdminDisplay.js – Full Updated Version with Organizer Tasks and Photobooth
...
const AdminDisplay = () => {
    ...
    const fetchStudentStatuses = async () => {
        try {
            ...
            snap.forEach(docSnap => {
                ...
                if (s.tookPhotoBooth) {
                    photobooth.push({ displayName: s.displayName, email: s.email });
                }
            });
    ...
    return (
        <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <h2 className="text-xl font-bold mt-10 mb-4 text-center">Photobooth Participants</h2>
            <div className="mb-6">
                <button
                    onClick={() => toggleDropdown("photo")}
                    className="w-full text-left bg-pink-100 hover:bg-pink-200 text-black font-semibold px-4 py-2 rounded-lg shadow transition"
                >
                    Photobooth Students ({photoBoothStudents.length})
                </button>

                {openDropdown === "photo" && (
                    <ul className="mt-2 border border-gray-200 rounded-lg p-4 bg-gray-50">
                        {photoBoothStudents.map((s, idx) => (
                            <li key={idx} className="text-sm text-gray-800 py-1">
                                <b>{s.displayName}</b> – <i>{s.email}</i>
                            </li>
                        ))}
